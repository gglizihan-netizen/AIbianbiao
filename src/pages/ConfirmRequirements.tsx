...
const getInitialFields = (isListUploaded: boolean, isDrawingsUploaded: boolean, projectType: string): Field[] => {
  const fields: Field[] = [
    {
      id: 'name',
      title: '项目名称',
...
export default function ConfirmRequirements() {
  ...
  const initialFields = React.useMemo(() => getInitialFields(isListUploaded, isDrawingsUploaded, projectType), [isListUploaded, isDrawingsUploaded, projectType]);
  ...
  return (
    <div className="h-screen bg-[#F5F7FA] font-sans flex flex-col overflow-hidden">
      ...
      <main className="flex-1 p-4 flex gap-4 overflow-hidden">
        ...
        <div className="flex-[3] flex flex-col gap-4 min-h-0 min-w-0">
          ...
          {initialParsingDone && (
            <div className="shrink-0 bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col p-6 gap-4">
              ...
              >
                ...
                <button className="text-gray-400 hover:text-gray-600 transition-colors w-6 h-6 flex items-center justify-center rounded-full hover:bg-gray-100">
                  <ChevronUp className={`transition-transform duration-300 ${!isOtherInfoExpanded ? 'rotate-180' : ''}`} size={18} />
                </button>
              </div>
